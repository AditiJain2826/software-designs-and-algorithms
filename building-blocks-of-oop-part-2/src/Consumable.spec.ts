...
describe("Consumable", () => {
  it("should not be spoiled", () => {
    expect(new ConsumableWithImplementation("bread", 1, 1).getisSpoiled()).toBeFalsy();
  });

  it("should be spoiled", () => {
    expect(new ConsumableWithImplementation("bread", 1, 1, true).getisSpoiled()).toBeTruthy();
  });

  describe("use()", () => {
    it("should return proper string when consumable is not consumed and spoiled", () => {
      expect(new ConsumableWithImplementation("bread", 1, 1, false).use()).toEqual("You consumed the bread.");
    });
